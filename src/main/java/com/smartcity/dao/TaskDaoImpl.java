...
public class TaskDaoImpl implements TaskDao {
    ...
    public Task create(Task task) {
        ...
        try {
            ...
            task.setId(Optional.ofNullable(holder.getKey().longValue()).map(Number::longValue)
    ...
    public List<Task> findByOrganizationId(Long id) {
        List<Task> taskList;
        try {
            taskList = this.jdbcTemplate.query(Queries.SQL_GET_BY_ORGANIZATION_ID, new TaskMapper(), id);
        } catch (Exception e) {
        ...
        if (taskList.isEmpty()) {
            logger.error("Can't get Task by Org id = {} Task Dao getByOrgId method error: ", id);
            throw new NotFoundException("Can't get Task by Org id = " + id + " Task Dao findByOrgId method error: ");
    ...
    public List<Task> findByUserId(Long id) {
        List<Task> taskList;
        try {
            taskList = this.jdbcTemplate.query(Queries.SQL_GET_BY_USER_ID, new TaskMapper(), id);
        } catch (Exception e) {
        ...
        if (taskList.isEmpty()) {
            logger.error("Can't get Task by User id = {} Task Dao findByUserId method error: ", id);
            throw new NotFoundException("Can't get Task by User id = " + id + " Task Dao findByUserId method error: ");
        } else return taskList;
    }
    ...
    private static class Queries {
        ...
        private static String SQL_GET_BY_USER_ID = "Select * from Tasks where users_organizations_id = " +
                "(Select id from Users_organizations Where organization_id = ?);";

        private static String SQL_GET_BY_ORGANIZATION_ID = "Select * from Tasks where users_organizations_id = " +
